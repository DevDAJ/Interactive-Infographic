...
(function () {
  ...
  let currentRadius = 3.6;
  let targetRadius = 3.6;
  camera.position.set(
  ...
  function markerPos(angleTop: number, angleSide: number): { x: number; y: number; z: number } {
    const r = 0.86;
  ...
  markerData.forEach((md) => {
    ...
    const ringG = new THREE.RingGeometry(0.10, 0.14, 28);
  ...
  tooltip.innerHTML = '<div class="bl-swatch"></div><div class="bl-info"><span class="bl-label"></span><span class="bl-hex"></span></div>';
  ...
  function onPointerMove(e: PointerEvent): void {
    ...
    if (pointerMoved > 4) {
      ...
      camElevation = Math.max(0.05, Math.min(1.2, dragStartElevation - dy * dragSpeed));
    }
  }
  ...
  sceneRoot.addEventListener('wheel', (e: WheelEvent) => {
    e.preventDefault();
  ...
  const presets = {
    sunset: { ball: '#f0e0c8', light: '#ff8844', sky: '#553388', platform: '#cc5555', rim: '#ffbb99' },
    studio: { ball: '#f0f0f0', light: '#ffffff', sky: '#8899cc', platform: '#666666', rim: '#ddeeff' },
    forest: { ball: '#e8f0e8', light: '#eeffcc', sky: '#559966', platform: '#2d7a2d', rim: '#ccffbb' },
    ocean: { ball: '#e0ecf5', light: '#cce0ff', sky: '#3377aa', platform: '#004488', rim: '#88ccff' },
  } as const;
  ...
  function animate() {
    ...
    camera.position.x = currentRadius * Math.cos(camElevation) * Math.sin(camAngle);
    camera.position.z = currentRadius * Math.cos(camElevation) * Math.cos(camAngle);
    ...
    markerMeshes.forEach((m, i) => {
      ...
      if (ring) {
        ...
        (ring.material as THREE.MeshBasicMaterial).opacity = 0.25 + Math.sin(markerTime + i * 2) * 0.2;
      }
      const glow = ud.glow;
      if (glow) {
        (glow.material as THREE.MeshBasicMaterial).opacity = 0.08 + Math.sin(markerTime + i * 2 + 1) * 0.07;
      }
      const isActive = activeMarker === m;
      (m.material as THREE.MeshBasicMaterial).color.set(isActive ? 0xffffff : 0x88bbff);
