...
export function getFolderSlidePx(): number {
  const raw = getComputedStyle(document.documentElement).getPropertyValue('--folder-slide').trim();
  const n = parseFloat(raw);
  return Number.isFinite(n) ? n : Math.min(window.innerHeight * 0.36, 280);
}

export function initFolderStack(): void {
  const stackPanels = gsap.utils.toArray('#hero, section:not(#hero)') as gsap.DOMTarget[];
