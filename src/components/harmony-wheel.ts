...
import { hexToRgb, rgbToHsl, hslToRgb, rgbToHex, normalizeHex, hexHue } from '../utils/color';
...
class HarmonyWheel extends HTMLElement {
  ...
  static _harmonyMeta(): Record<HarmonyKind, { title: string; body: string }> {
    return {
      complementary: {
        title: 'Complementary',
        body:
          'Two colors opposite each other on the wheel. They create strong contrast and visual energy - ideal for accents, calls to action, and dynamic layouts. Classic pairs include red ↔ green, blue ↔ orange, and yellow ↔ purple.',
      },
      analogous: {
        title: 'Analogous',
        body:
          'Three hues that sit side by side on the wheel. The effect is calm and cohesive - common in nature (sunsets, forests) and great for branding that needs harmony without high tension.',
      },
      triadic: {
        title: 'Triadic',
        body:
          'Three colors evenly spaced around the wheel (every 120°). You get plenty of vibrancy while staying balanced - one color often leads, with the other two as support.',
      },
    };
  }

  static _segmentPath(cx: number, cy: number, rOuter: number, rInner: number, a1: number, a2: number): string {
    const span = a2 - a1;
  ...
  connectedCallback() {
    this._segmentIndex = parseInt(this.getAttribute('start-segment') || '0', 10);
    ...
    if (h === 'analogous' || h === 'triadic' || h === 'complementary') this._harmony = h;
  ...
  disconnectedCallback() {
    ...
    this.shadowRoot?.querySelectorAll('.seg-fill').forEach((path) => gsap.killTweensOf(path));
  }
   ...
   */
  _tweenPathFill(path: SVGPathElement, toHex: string, opts: { instant?: boolean } = {}): void {
    const instant = opts.instant === true || typeof gsap === 'undefined' || this._reduceMotion();
    const rawFrom = path.getAttribute('fill') || path.getAttribute('data-base-fill') || '#000000';
      ...
      btn.addEventListener('click', () => {
        ...
        if (id === 'complementary' || id === 'analogous' || id === 'triadic') {
          ...
          root.querySelectorAll('[data-harmony-btn]').forEach((b) => {
            b.setAttribute('aria-pressed', b.getAttribute('data-harmony-btn') === id ? 'true' : 'false');
          });
          this._syncDesc();
  ...
  _nudgePulse() {
    ...
    hi.forEach((i) => {
      const g = root.querySelector(`.seg-group[data-seg="${i}"]`) as SegPulseGroup | null;
      if (!g) return;
  ...
  _refreshSegmentVisuals(_instant = false): void {
    const root = this.shadowRoot;
    if (!root) return;
    const hi = new Set(HarmonyWheel._harmonyIndices(this._segmentIndex, this._harmony));
    const layer = root.querySelector('.wheel-layer');

    root.querySelectorAll('.seg-group').forEach((g) => {
      if (typeof gsap !== 'undefined') gsap.set(g as gsap.DOMTarget, { clearProps: 'transform' });
    });
  ...
  _updateRing(_instant = false): void {
    ...
    const seg = (2 * Math.PI) / n;
    const ringD = HarmonyWheel._segmentPath(cx, cy, ringR, ringIR, -Math.PI / 2, -Math.PI / 2 + seg);
    ...
    let existing = [...container.querySelectorAll('.sel-ring-item')] as SVGGElement[];
  ...
  _updateArrows() {
    ...
    const arrowTag = (_parent?: SVGElement | null) => {
      ...
      l.setAttribute('x2', String(cx)); l.setAttribute('y2', String(headBaseY));
      l.setAttribute('stroke-width', String(sw)); l.setAttribute('stroke-linecap', 'round');
      ...
      g.appendChild(l); g.appendChild(hd);
      g.style.transformOrigin = `${cx}px ${cy}px`;
      if (_parent) _parent.appendChild(g);
      return g;
    };

    let existing = [...container.querySelectorAll('.arrow-item')] as SVGGElement[];
  ...
  _stackHarmonyOnTop(layer: SVGElement, hi: Set<number>): void {
    ...
    nodes.forEach((n) => layer.appendChild(n as SVGElement));
  }
  ...
  _syncDesc() {
    ...
    if (chipEl) {
      const indices = HarmonyWheel._harmonyIndices(this._segmentIndex, this._harmony);
      const sorted = [...indices];
      const baseIdx = sorted.indexOf(this._segmentIndex);
      if (baseIdx > 0) sorted.splice(baseIdx, 1), sorted.unshift(this._segmentIndex);
      const chipsHtml = sorted.map((i) => {
  ...
  <path class="seg-fill" data-base-fill="${c}" d="${d}" fill="${c}" stroke="none" stroke-linejoin="round" paint-order="stroke fill"/>
    ...
    const buttons = (['complementary', 'analogous', 'triadic'] as HarmonyKind[]).map((key) => {
      const m = meta[key];
