import { hexHue, hsvToRgb, rgbCss, luminance } from '../utils/color';
...
class ColorWheel extends HTMLElement {
  ...
  attributeChangedCallback(name: string, prev: string | null, next: string | null) {
    if (name === 'size' && prev !== next && this.shadowRoot?.querySelector('svg')) {
      this.render();
      this._bind();
      this._syncFromSliders();
      this._updateVisuals();
  ...
  _bind(): void {
    ...
    if (!root) return;
    const hEl = root.querySelector('[data-slider="h"]') as HTMLInputElement | null;
    const sEl = root.querySelector('[data-slider="s"]') as HTMLInputElement | null;
    const vEl = root.querySelector('[data-slider="v"]') as HTMLInputElement | null;
    const onChange = () => {
    ...
    [hEl, sEl, vEl].forEach((el) => el?.addEventListener('input', onChange));
