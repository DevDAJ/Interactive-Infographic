...
import { normalizeHex, avgColor } from '../utils/color';
...
class ColorMixer extends HTMLElement {
  ...
  _killDrags() {
    this._drags.forEach((d) => d.kill());
    this._drags = [];
  }

  _chipHtml(hex: string, d: number, className: string, extraAttrs = ''): string {
  ...
  _preview(): { hex: string | null; depth: number; mode: 'mix' | 'single' | 'empty' } {
  ...
  _tierUI(depth: number, mode: 'mix' | 'single' | 'empty'): { label: string; kind: string } {
  ...
  render(): void {
    ...
    const { slotA, slotB, primaryColors, secondaryColors, extraStrip, showSecondaries, palette } = this.state;
    const prev = this._preview();
    const tier = this._tierUI(prev.depth, prev.mode);

    const primaryHtml = primaryColors.map((h) => this._chipHtml(h, 0, 'strip-chip')).join('');
    const secondaryHtml = secondaryColors.map((h) => this._chipHtml(h, 0, 'strip-chip')).join('');
    const extraHtml = extraStrip.map((h) => this._chipHtml(h, 0, 'strip-chip')).join('');
...
<div class="wrap">
  ...
  _closestSlot(el: HTMLElement, slotA: HTMLElement, slotB: HTMLElement): 'a' | 'b' | null {
    ...
    requestAnimationFrame(() => {
      ...
      draggables.forEach((chipEl) => {
        ...
        const dragVars = {
          ...
          onDragStart() {
            gsap.to(el, { scale: 1.12, duration: 0.15, ease: 'power2.out', overwrite: 'auto' });
          },
          onDragEnd() {
            ...
            if (hit) {
              mixer._assignSlot(hit, { hex: normalizeHex(hex ?? '#000000'), d });
              if (swatch) {
                gsap.fromTo(swatch, { scale: 1 }, { scale: 1.06, duration: 0.18, yoyo: true, repeat: 1, ease: 'power2.inOut' });
              }
            } else {
              gsap.to(el, { x: 0, y: 0, scale: 1, duration: 0.25, ease: 'power3.out', overwrite: 'auto' });
            }
          },
        };
